...
export class ForecastListComponent {
  ...
  extractTimeseries = (forecast$: Observable<any>): Observable<TimeSeriesEntry[]> => {
    return forecast$.pipe(map((forecast: any) => {
      return forecast.properties?.timeseries as TimeSeriesEntry[]
    }));
  }

  filterEverySixthHour = (timeseries$: Observable<TimeSeriesEntry[]>): Observable<TimeSeriesEntry[]> => {
    return timeseries$.pipe(
      map((timeseries: TimeSeriesEntry[]) => timeseries.filter((timeseriesEntry) => !(new Date(timeseriesEntry.time).getUTCHours() % 6)))
    );
  }

  groupByDate = (timeseries$: Observable<TimeSeriesEntry[]>): Observable<Map<string, TimeSeriesEntry[]>> => {
    return timeseries$.pipe(
  ...
  generateDayViewModel = (dateGroups$: Observable<Map<string, TimeSeriesEntry[]>>): Observable<ForecastDay[]> => {
    return dateGroups$.pipe(
  ...
  $forecastDays: Observable<ForecastDay[]> = this.weatherService.$forecast.pipe(
    ...
    this.filterEverySixthHour,
    this.groupByDate,
    this.generateDayViewModel
  )
}
